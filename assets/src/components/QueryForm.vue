  ...
  import 'datatables/media/js/jquery.dataTables.min.js';
    ...
    var render_categories = function(data) {
        ...
        if ("Error" in data) {
            ...
            console.log(data);
            $("#country-results").append('<br><h3><a href="' + data["article"] + '">' + document.getElementById('title').value + '</a></h3>');
            $("#country-results").append('<ol>');
            $("#country-results").append('<li><b>Total Number of Outlinks: ' + data['results']['outlink_count']);
            $("#country-results").append('<li><b>Total Number of Inlinks: ' + data['results']['inlink_count']);
            $("#country-results").append('</ol>');

            if (data['results']['outlink_summary']['regions'].length > 0){
                $("#country-results").append('<br><h2> Outlink Summary </h2>')
                $("#country-results").append('<div class="link-legend"><div class="legend-title">Outlink Threshold Guide</div><div class="legend-scale"><ul class="legend-labels"><li><span style="background:#bae4bc;"></span>Above</li><li><span style="background:#fdae6b;"></span>Below</li></ul></div></div>')
                $("#country-results").append('<table id="outlink-summary-table" class="display"><thead><tr><th>Region</th><th>Outlink Count</th><th>Percentage distribution of outlinks (%)</th></tr></thead></table>')
            }else{
            ...
            if (data['results']['inlink_summary']['regions'].length > 0){
                ...
                $("#country-results").append('<div class="link-legend"><div class="legend-title">Inlink Threshold Guide</div><div class="legend-scale"><ul class="legend-labels"><li><span style="background:#bae4bc;"></span>Above</li><li><span style="background:#fdae6b;"></span>Below</li></ul></div></div>')
                $("#country-results").append('<table id="inlink-summary-table" class="display"><thead><tr><th>Region</th><th>Inlink Count</th><th>Percentage distribution of inlinks (%)</th></tr></thead></table>')
            }else{
    ...
    function queryCountryAPI() {
        if (document.getElementById('lang').value && !document.getElementById('title').value) {
        ...
        var queryUrl = "/api/v1/get-summary?lang=" + document.getElementById('lang').value + "&title=" + document.getElementById('title').value +"&threshold="+document.getElementById('threshold').value;
